// msft post and fetch comment 

// Variables to keep track of pagination
var msftcurrentPage = 1;
var msftcommentsPerPage = 5;

// function for fetching comment 
function fetchmsftComments() {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', '/fetchmsftComments', true);
    ...
    xhr.onload = function() {
        if (xhr.status === 200 && xhr.responseText) {
            ...
            var startIndex = (msftcurrentPage - 1) * msftcommentsPerPage;
            var endIndex = startIndex + msftcommentsPerPage;
            ...
            var msftCommentsContainer = document.getElementById('msftcommentsContainer');
            msftCommentsContainer.innerHTML = commentsHtml;
        }
    };
    xhr.send();
}
...
document.getElementById('msftnextPageBtn').addEventListener('click', function() {
    msftcurrentPage++;
    fetchmsftComments();
});

// Event listener for previous page button
document.getElementById('msftprevPageBtn').addEventListener('click', function() {
    if (msftcurrentPage > 1) {
        msftcurrentPage--;
        fetchmsftComments();
    }
});
...
var msftForm = document.getElementById('msftCommentForm');
msftForm.addEventListener('submit', function(e) {
    e.preventDefault();
    var formData = new FormData(msftForm);
    var xhr = new XMLHttpRequest();
    xhr.open(msftForm.method, msftForm.action, true);
    xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
    xhr.onload = function() {
        if (xhr.status === 200 && xhr.responseText) {
            var response = JSON.parse(xhr.responseText);
            if (response.success) {
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Success',
                    text: 'Comment added successfully MSFT.',
                });
                ...
                '</div>' +
                '<div class="comment" style="">' + 
                '<p style="color:white; word-wrap: break-word; vertical-align: middle;">' + formData.get('msftComment') + '</p>' +
                '</div>' +
                '</div>';
            document.getElementById('msftcommentsContainer').insertAdjacentHTML('beforeend', newComment);
            } else {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
